...
func printSARIF(findings []Finding) {
	...
	rules := []SarifRule{
		...
		},
		{
			...
			FullDescription:  SarifMessage{Text: "Detects when secret data is passed to logging, printing, or error formatting functions. Secrets may be exposed in logs, console output, or error messages. Authenticators such as HMAC output are excluded: they are published by design."},
		...
		},
		{
			...
			FullDescription:  SarifMessage{Text: "Detects when secret data is sent to or received from channels. Channel communication can leak timing information and create attack surfaces where goroutines can observe secret data. Authenticators such as HMAC output are excluded: they are published by design."},
		...
		},
		{
			...
			FullDescription:  SarifMessage{Text: "Detects when secret-tainted values flow into network, file, or syscall I/O functions inside //ctguard:isolated regions. Isolated regions are code sections that must not leak secrets through I/O operations. Authenticators such as HMAC output are excluded: they are published by design."},
